...
YAML="$REPO_ROOT/config/tools.yaml"
STATE="$REPO_ROOT/config/setup-state.json"
JSON_OUT=0
[[ "${1:-}" == "--json" ]] && JSON_OUT=1
...
run_check() {  # check 命令在仓库根执行; 不打印任何密钥
  local cmd="$1"
...
if [[ $JSON_OUT -eq 1 ]]; then
  ...
  echo "（状态已写入 config/setup-state.json；接入手册见 config/setup-docs/）"
fi
