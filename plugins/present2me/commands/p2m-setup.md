---
description: present2me 依赖预检（d2/node/viewer 构建/工具授权状态），缺什么补什么
---
...
3. **Excalidraw 查看器未构建时**（`<插件根>/tools/excalidraw-viewer/viewer.js`
   不存在）：确认后在 `<插件根>/tools/excalidraw-viewer/` 执行
   `npm install --no-fund --no-audit && npm run build`。
4. **node/npm 缺失时**：提示安装 Node.js（brew install node），跳过步骤 3。
5. 挂账工具（Anki/思源/Notion/Flomo/GDocs）不强求开通，只提示手册位置：
   https://github.com/Kozmosa/present2me/tree/main/config/setup-docs
6. 复跑 `tools/status.sh` 确认 viz 类工具全部 ✅，汇总结果给用户。
