...
| `tools/` | docs.sh（文档站，实体）+ status.sh / render-d2.sh / build-excalidraw.mjs / validate-excalidraw.mjs / fix-excalidraw-text.mjs / excalidraw-viewer（软链 → 插件 tools/）+ release.sh（发版） |
...
2. **讲解默认可视化**：讲复杂概念时按 explain-concept 的选型表配图；
   产物落盘（artifacts/ 或 scratch/），渲染后打开给用户看（ZCode 内按下方
   「ZCode 环境偏好」用应用内浏览器），据反馈迭代。
...
- **预览用应用内浏览器**：ZCode 会话中打开渲染产物（D2 SVG、Excalidraw
  查看器、docs 站等）时，优先用 ZCode 应用内浏览器（in-app browser），
  不拉起系统默认浏览器（脚本里 `open` 的行为留给非 ZCode 环境）。
- **看图走视觉子代理**：需要查看图像内容（截图、渲染结果、UI 效果检查）时，
  派发 general-purpose 子代理（由具备视觉能力的模型驱动）读取并描述图像，
  不凭文件名或猜测下结论。
